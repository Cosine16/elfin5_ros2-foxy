...
> 工作空间：厂家驱动层 `~/cos_ws/elfin_ws`（underlay）+ 自研应用层 `~/cos_ws/app_ws`（overlay，colcon）
> 使用时只需 source `app_ws/install/setup.bash`（自动串接 elfin_ws）
> 相关问题清单见 [[已知问题]]
...
| 区域 | 内容 | 是否进 ROS |
|---|---|---|
| `elfin_ws/src/elfin_robot/` | 华沿官方 ROS2 包（仿真 + 实机驱动，共 10 个包） | 是（underlay） |
| `app_ws/src/cos_shape/`、`app_ws/src/cos_rsvisual/` | 自研应用包（末端轨迹演示、视觉跟随） | 是（overlay） |
| `scripts/` | 启动/构建/等待脚本，`legacy/` 为实机时代归档 | 否 |
| `third_party/windows_sdk/`、`document/` | Windows 侧 C++ SDK（TCP 直连控制柜）、官方文档 | 否 |
...
##### 一键脚本
```bash
~/cos_ws/scripts/start_sim.sh          # moveit2（容器内自动后台，日志 app_ws/log/sim/）
~/cos_ws/scripts/start_sim.sh gazebo   # 再起 Control Panel
~/cos_ws/scripts/start_sim.sh stop     # 停止
```
注意：`start_sim.sh` 不起 basic_api；`start_sim.py` 起全部 3 个但需要 gnome-terminal。

#### 真机（EtherCAT，需 PREEMPT_RT 内核）

严格按顺序开 4 个终端（等价于 `scripts/start_real.py`，旧版见 `scripts/legacy/opcode.bash`）：
...
```
> 硬件终端需要 cap_net_raw（EtherCAT 裸套接字）与 cap_sys_nice。
> `scripts/start_real.py` 默认用 `sudo capsh` 以**普通用户身份 + ambient
> capabilities** 启动该终端（不要用 root 跑全栈：Fast DDS 的 SHM 段会挡住
> 普通用户终端，见 [[已知问题]] 第 18 条）。
