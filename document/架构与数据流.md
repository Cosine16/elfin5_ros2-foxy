...
> 工作空间：`~/cos_ws/elfin_ws`（colcon），源码在 `elfin_ws/src/`
> 相关问题清单见 [[已知问题]]
...
##### 一键脚本
```bash
~/cos_ws/scripts/start_sim.sh          # moveit2（容器内自动后台，日志 elfin_ws/log/sim/）
...
严格按顺序开 4 个终端（等价于 `scripts/legacy/start_real.py` / `opcode.bash`）：
