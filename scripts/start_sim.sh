...
#   SIM_WS=<path>   覆盖工作空间路径（默认 <脚本目录>/../elfin_ws）
# =====================================================================
set -euo pipefail
...
# 默认工作空间 = 本脚本所在目录(cos_ws/scripts)的上一级下的 elfin_ws。
# 用相对脚本位置解析（而非写死 $HOME/cos_ws），整仓移动到任意路径都能找到构建产物。
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WS="${SIM_WS:-$SCRIPT_DIR/../elfin_ws}"
